"""ML Service v2.1 — XGBoost + SMOTE + SHAP"""
...
DEPT_MAP = {"CS":0,"IS":1,"EC":2,"EE":3,"ME":4,"CG":5}
...
def _load_artifacts():
    ...
    pp = os.path.join(ML_DIR,"risk_pipeline.pkl")
    ep = os.path.join(ML_DIR,"shap_explainer.pkl")
    mp = os.path.join(ML_DIR,"model_meta.json")
    if os.path.exists(pp): _pipeline = joblib.load(pp)
    if os.path.exists(ep): _explainer = joblib.load(ep)
    if os.path.exists(mp):
        with open(mp) as f: _model_meta = json.load(f)
    if _pipeline: logger.info("✅ ML pipeline loaded.")
...
def train_model(df: pd.DataFrame) -> Dict:
    ...
    spw = max(1, int((y==0).sum() / max((y==1).sum(),1)))
    ...
    fi = [{"feature":f,"importance":round(float(v),5)}
          for f,v in sorted(zip(FEATURE_COLS,np.abs(sv).mean(axis=0)),key=lambda x:-x[1])]
    ...
    meta = {
        ...
        "n_samples":     int(len(y)),
        "n_at_risk":     int(y.sum()),
        "confusion_matrix": confusion_matrix(y, best_preds).tolist(),
    ...
    with open(os.path.join(ML_DIR,"model_meta.json"),"w") as f: json.dump(meta,f,indent=2)
    _pipeline = pipeline; _explainer = explainer; _model_meta = meta
    logger.info(f"✅ Model trained: F1={meta['f1_score']:.4f}")
    return meta


def get_model_meta() -> Dict:
    if not _model_meta: _load_artifacts()
    return _model_meta


def predict_students(students_data: List[Dict]) -> List[Dict]:
    global _pipeline, _explainer, _model_meta
    if _pipeline is None: _load_artifacts()
    if _pipeline is None: raise RuntimeError("No trained model. Call /api/ml/train first.")

    df = pd.DataFrame(students_data)
    df["dept_enc"] = df["department"].map(DEPT_MAP).fillna(0).astype(int)
    df["extracurricular_participation"] = df.get("extracurricular_participation",
                                                   pd.Series([0]*len(df))).fillna(0).astype(int)
    ...
    sv = None
    if _explainer:
        try: sv = _explainer.shap_values(X)
        except: pass

    results = []
    for i in range(len(df)):
        p = float(proba[i])
        level = "Critical" if p>=0.75 else "High" if p>=0.55 else "Medium" if p>=0.30 else "Low"
        top_factors = []
        if sv is not None:
            for idx in np.argsort(np.abs(sv[i]))[::-1][:5]:
                top_factors.append({
                    "feature": FEATURE_COLS[idx],
                    "shap_value": round(float(sv[i][idx]),4),
                    "impact": "increases" if sv[i][idx]>0 else "decreases",
