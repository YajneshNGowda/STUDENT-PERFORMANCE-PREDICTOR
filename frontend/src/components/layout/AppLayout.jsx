...
function UserAvatar({ user }) {
  ...
  return (
    <div className="flex items-center justify-center h-8 w-8 rounded-lg bg-brand-600 text-white text-xs font-bold select-none">
      {initials}
    </div>
  )
}
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="flex items-center gap-3 px-5 py-5 border-b border-surface-border dark:border-surface-dark-border">
        ...
        <div>
          ...
          <div className="text-xs text-slate-500 dark:text-slate-400">Risk Intelligence</div>
        </div>
      </div>
      ...
      <div className="px-4 py-3 border-b border-surface-border dark:border-surface-dark-border">
        <div className="flex items-center gap-3 p-2 rounded-lg bg-surface-secondary dark:bg-surface-dark-tertiary">
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="text-xs text-slate-500 dark:text-slate-400">{ROLE_LABELS[user?.role]}{user?.department_code ? ` · ${user.department_code}` : ''}</div>
          </div>
        </div>
      </div>
        ...
        <header className="h-16 flex items-center gap-4 px-4 lg:px-6 bg-white dark:bg-surface-dark-secondary border-b border-surface-border dark:border-surface-dark-border flex-shrink-0">
          ...
          <div className="relative">
            <button onClick={() => setUserMenuOpen(o => !o)}
              ...
              <UserAvatar user={user} />
              <span className="text-sm font-medium text-slate-700 dark:text-slate-300 hidden sm:block">{user?.full_name?.split(' ')[0]}</span>
            ...
            {userMenuOpen && (
              <div className="absolute right-0 top-full mt-1 w-52 card py-1 z-20 shadow-lg">
                ...
                <NavLink to="/settings" onClick={() => setUserMenuOpen(false)} className="flex items-center gap-2 px-3 py-2 text-sm hover:bg-surface-secondary dark:hover:bg-surface-dark-tertiary dark:text-slate-300">
                  <Settings className="h-4 w-4" /> Settings
                </NavLink>
